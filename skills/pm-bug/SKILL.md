...
3. **Affected customers** — optional, search Customers DB `collection://2e8268e7-4af8-800f-a65c-000be39698a3`
4. **Fix type** — auto-derive from priority (see below)
5. **Assignee** — fetch Notion users and ask
6. **Sprint** — search Sprint DB `collection://2d9268e7-4af8-80ac-83b2-000b6dcef569`, pick the most recent date ≤ today
7. **回報時間（客戶實際回報時間）** — **一律用 `AskUserQuestion` 問使用者**「客戶實際回報這個 bug 的時間」（日期 + 時分）。**禁止 silently 用目前時間 / 建卡時間**——客戶回報時間系統抓不到，建卡時間 ≠ 回報時間，DORA MTTR 從「回報」起算，用錯起點全失真。不確定時填估計值並在 meta `timeline.reported_at` 標 `# 待確認`。寫進 Notion 時 **API 設 `is_datetime: 1`**（純日期 → datetime），否則只存日期、時間全丟：
   ```
...
> ⚠️ **寫任何 Bug 卡內文前必讀 `shared/notion-card-writing-style.md`**（單一事實來源，Story / Bug 共用）。

重點摘要（完整規則與改寫對照範例見該檔）：

- **開卡時**（問題描述 / 重現步驟）：用使用者視角——什麼畫面、按了什麼、看到什麼。`名稱🖍️` 用行為描述（✅「儲存公告後附件消失」／❌「公告附件與 metadata 無法儲存」）。
- **修復後**（`技術描述` 欄位、Bug page「根本原因」section、開發/緊急修復 Task 完成說明）：用固定五段——`## 問題是什麼` / `## 為什麼會這樣` / `## 連帶找到的問題`（無則省略）/ `## 怎麼修的` / `## 驗證方式`。
- **禁止**：程式碼片段、變數 / 函式 / DTO 名、檔案路徑、行號、資料表 / view 名、端點路徑、框架術語堆疊（TypeORM / Prisma / Hasura / client-side…）。這些留 git commit message、PR description、`fixes/<slug>/context.md`。
- **例外**：`技術描述` 欄位本身可用術語（那是給 dev 的精確名稱）。
- **判準**：把卡片拿給不寫 code 的同事看，他能不能講出「這張卡在做什麼、為什麼要做、做完會怎樣」。
...
⚠️ **這條規則不限於正式呼叫 `/pm-bug` 的場合。** 任何幫 bug 卡補 Task 的情境——手動用 `notion-api.sh` 回填、git 考古後補卡、驗收巡查時補卡——都要照這張表建完整 pipeline，不能只建當下狀態對應的單一階段（例如只建「開發」卡就當補完）。尚未修復的 bug 一樣要把後續階段（人工測試/更版/驗收）一次建齊，狀態設「即將進行」，不是等到真的做到那個階段才回頭補。
